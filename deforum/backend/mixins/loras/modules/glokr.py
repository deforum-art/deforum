...
class LokrModule(nn.Module):
    ...
    def set_weight(self, multiplier):
        self.multiplier = multiplier
        self.weight = self.org_module[0].weight.data + self.get_weight(self.org_module[0].weight.data) * self.multiplier
        self.bias = None if self.org_module[0].bias is None else self.org_module[0].bias.data
        return self
    ...
    def forward(self, x):
        return self.op(x, self.weight.view(self.shape), self.bias, **self.extra_args)
