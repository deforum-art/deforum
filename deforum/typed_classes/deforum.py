...
class DeforumConfig(DefaultBase):
    ...
    def validate_xformers (cls, value, values, config, field):
        if value:
            if not is_xformers_available():
                raise ImportError("xformers is not installed. Please install it following the instructions on 'https://github.com/facebookresearch/xformers'")
            else:
                return True
        else:
            return False
    ...
    def validate_pytorch_optimizations (cls, value, values, config, field):
        if value:
            from deforum.utils.pytorch_optimizations import \
    ...
    def validate_set_use_flash_attn_2 (cls, value, values, config, field):
        if value:
            try:
                ...
                raise ImportError("flash_attn_2_cuda is not installed. Please install it following the instructions on 'https://github.com/Dao-AILab/flash-attention'")
        else:
            return False