...
from deforum.datafunctions.rich import console
#Webui
...
def generate_inner(args, keys, anim_args, loop_args, controlnet_args, root, frame = 0, return_sample=False, sampler_name=None):
    ...
    if processed is None:
        ...
        assert not ( (mask is not None and args.use_mask and args.overlay_mask) and (root.init_sample is None and init_image is None)), "Need an init image when use_mask == True and overlay_mask == True"
...
def print_combined_table(args, anim_args, p, keys, frame_idx):
    from deforum.datafunctions.rich import Table
    from deforum.datafunctions.rich import box
